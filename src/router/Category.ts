...
categoryRouter.delete('/', async (req, res) => {
  try {
    ...
    console.error('Erreur lors de la suppression de toutes les catégories :', error);
    res.status(500).json({ error: 'Erreur interne du serveur' });
  }
});
...
categoryRouter.put('/:id', async (req, res) => {
  try {
    ...
    if (!name || !description) {
      return res.status(400).json({ error: 'Veuillez fournir le nom et la description pour la mise à jour.' });
    }
    ...
    if (categoryToUpdate) {
      ...
      res.status(200).json({ message: 'La catégorie a été modifiée avec succès.' });
    } else {
